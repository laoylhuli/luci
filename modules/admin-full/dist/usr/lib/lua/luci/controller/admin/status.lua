...
function index()
	...
	entry({"admin", "status", "overview"}, template("admin_status/index"), _("About Device"), 1)	--关于设备
	--entry({"admin", "status", "iptables"}, call("action_iptables"), _("Firewall"), 2).leaf = true
	entry({"admin", "status", "routes"}, template("admin_status/routes"), _("Routes"), 3)	--路由表
	--entry({"admin", "status", "syslog"}, call("action_syslog"), _("System Log"), 4)
	--entry({"admin", "status", "dmesg"}, call("action_dmesg"), _("Kernel Log"), 5)
	--entry({"admin", "status", "processes"}, cbi("admin_status/processes"), _("Processes"), 6)

	entry({"admin", "status", "realtime"}, alias("admin", "status", "realtime", "load"), _("Realtime Graphs"), 7)	--实时信息
